    ...
    */
    if( false === $args['show_posts_links'] ) {
        ...
        if( 1 === $max_pages ) {
            ...
            */
            foreach ( $range_numbers as $v ) {
                ...
                if ( $v == $current_page ) { 
                    ...
                    $page_numbers[] = '<li class="current disabled"><a href="#">' . $v . '</a></li>';

                }else{

                    $page_numbers[] = '<li><a href="' . get_pagenum_link( $v ) . '" class="inactive">' . $v . '</a></li>';
            ...
            */
            $previous_page  = ( $current_page !== 1 )                       ? '<li><a href="' . get_pagenum_link( $current_page - 1 ) . '">' . $args['previous_page_text'] . '</a></li>' : '';
            $next_page      = ( $current_page !== $max_pages )              ? '<li><a href="' . get_pagenum_link( $current_page + 1 ) . '">' . $args['next_page_text'] . '</a></li>'     : '';
            $first_page     = ( !in_array( 1, $range_numbers ) )            ? '<li><a href="' . get_pagenum_link( 1 ) . '">' . $args['first_page_text'] . '</a></li>'                    : '';
            $last_page      = ( !in_array( $max_pages, $range_numbers ) )   ? '<li><a href="' . get_pagenum_link( $max_pages ) . '">' . $args['last_page_text'] . '</a></li>'            : '';
            ...
            */
            ...
            $paginated_text = '<nav class="text-center"><ul class="pagination pagination-sm">';
            $paginated_text .= $first_page . $previous_page . $numbers_string . $next_page . $last_page;
            $paginated_text .= '</ul></nav>';
